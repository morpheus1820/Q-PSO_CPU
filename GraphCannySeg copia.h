...
template <>
void min_max(image<rgb> *im, rgb *ret_min, rgb *ret_max) {
    int width = im->width();
    int height = im->height();
...
/* compute minimum and maximum value in an image
 Overload for rgb datatype NO IF-else in compile time*/
template <>
void min_max(image<CIELab> *im, CIELab *ret_min, CIELab *ret_max) {
    int width = im->width();
    int height = im->height();
...
inline void universe::getPixelCoordFromIdx(int idx, int w,
                                           int& x, int& y)
{
    x = (idx % w);
...
//        float k_vec[9] = {571.9737, 0, 319.5000, 0, 571.0073, 239.5000, 0,0,1};
    ...
    template <class Q>
    image<Q>* convertMat2Image(const cv::Mat& m)
    {
        int  width = m.cols;
        int  height = m.rows;
...
//            }
    ...
    template <class Q>
    static void visualizeImage(image<Q> *im, std::string win_name="Segmentation Result", int wait_key_=0, bool bInpaintDepth=false)
    {
    ...
    static void dynamicDepthSmoothing(image<uint16_t> *im, image<uint16_t> * smoothedDepth, const float beta = 1500.0f, const float gamma = 18000.0f)
    {
...
//        return im;
    ...
    static void myCannyWithOri2(const cv::Mat& grayImg,
...
//                imRef(mInpaintedDepth,xyi[idx_set][i].x,xyi[idx_set][i].y); //r
            ...
            //Save the results
            SegResults segres_(Centroid3D, cntr, eigen_val[0], eigen_val[1], angle, xyi[idx_set].size(), xyi[idx_set], clusterRGBmat,clusterDepthmat);
...
//            float weight = (mKy*dist_hsv*dist_hsv*dist_hsv*dist_hsv + mKx*DeltaDepth*std::log2(1.0f+dist_hsv) + /*mKs*std::log2(1.0f+ds))*/mKs*ds)/(mKy+mKx+mKs);
    ...
    static void visualizeColorMap(const cv::Mat& oneCHimg, std::string name="W1", int wait_key_=0, bool getgrayscale=false)
    {
    ...
    static void convertDepth2ColorMap(const cv::Mat& oneCHimg, cv::Mat& dpCM)
    {
        //if already 8 bit
...
//            printf("%s: min: %f, max: %f\n",name.c_str(),minVal, maxVal);
    ...
    void computeValueHist(const cv::Mat& image, bool isHSV, const cv::Mat& mask, cv::Mat_<float>& Val_HistNorm, cv::Mat& histImage,int clusterArea,int Hbins=256, bool viz_=true )
    {
    ...
    void computeSatHist(const cv::Mat& image, bool isHSV, const cv::Mat& mask, cv::Mat_<float>& Sat_HistNorm, cv::Mat& histImage,int Hbins=256, bool viz_=true )
    {
    ...
    void computeHueHist(const cv::Mat& image, bool isHSV ,const cv::Mat& mask, cv::Mat_<float>& Hue_HistNorm, cv::Mat& histImage,int Hbins=180, bool viz_=true )
    {
